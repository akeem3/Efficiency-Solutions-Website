...
## 🎯 Deliverables
- [ ] A dedicated `CartDrawer` component in `src/components/layout/`.
- [ ] Animated item list (Framer Motion `LayoutGroup`).
- [ ] Empty state with "Continue Discovery" call-to-action.
...
## ✅ Acceptance Criteria
- [ ] Cart drawer is fully accessible via keyboard (`Esc` to close, `Tab` focus for items).
- [ ] The "Empty Cart" view appears if `totalCount` is 0.
- [ ] Swipe-to-dismiss works intuitively on mobile devices.
- [ ] Item quantities can be manipulated directly from within the drawer.
