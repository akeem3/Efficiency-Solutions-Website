...
export const ProductCard = ({ product, className }: ProductCardProps) => {
  ...
  return (
    <Card className={cn("group overflow-hidden border-none bg-surface shadow-sm transition-all duration-300 hover:shadow-md", className)}>
      <div className="relative aspect-square overflow-hidden bg-brand-muted/10">
        <Image
        ...
        {images.length > 1 && (
          <div className="absolute inset-0 flex items-center justify-between px-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button 
              onClick={prevImage}
              className="p-1 rounded-full bg-white/80 text-brand-primary backdrop-blur-sm shadow-md transition-transform hover:scale-110"
            >
