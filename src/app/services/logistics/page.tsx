...
export default function LogisticsPage() {
  ...
  const [filteredVehicles, setFilteredVehicles] = useState<LogisticsProduct[]>(MOCK_LOGISTICS_PRODUCTS);

  useEffect(() => {
    let result = [...MOCK_LOGISTICS_PRODUCTS];
  ...
  return (
    <div className="min-h-screen bg-brand-background px-6 py-12 lg:px-12">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="relative mb-16 overflow-hidden rounded-[2.5rem] bg-brand-primary p-10 lg:p-20 text-white shadow-2xl">
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 relative z-10">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-secondary/20 rounded-full text-brand-secondary text-[10px] font-bold uppercase tracking-widest mb-6 border border-brand-secondary/30">
              ...
              <div className="flex gap-4">
                 <div className="px-6 py-3 bg-white/10 rounded-2xl backdrop-blur-md border border-white/10">
                    <span className="block text-2xl font-bold font-heading text-brand-secondary">24/7</span>
                    <span className="text-[10px] uppercase font-bold text-white/50 tracking-widest">Rapid Support</span>
                 </div>
                 <div className="px-6 py-3 bg-white/10 rounded-2xl backdrop-blur-md border border-white/10">
                    <span className="block text-2xl font-bold font-heading text-brand-secondary">100%</span>
                    <span className="text-[10px] uppercase font-bold text-white/50 tracking-widest">Private Fleet</span>
                 </div>
              </div>
            </motion.div>
            ...
            >
              <Image
                ...
                alt="Fleet Showcase"
                fill
                className="rounded-3xl object-cover shadow-2xl brightness-90 contrast-110"
                priority
              />
        ...
        <div className="flex flex-col gap-10 lg:flex-row xl:gap-14">
          ...
          <div className="flex-1">
            {/* Fleet Controls */}
            <div className="mb-12 flex flex-col items-center justify-between gap-6 rounded-[2rem] bg-white p-8 shadow-2xl ring-1 ring-black/5 md:flex-row md:px-12">
              <div className="relative w-full md:max-w-lg">
                ...
                <Input 
                  ...
                  className="h-14 w-full rounded-full border-none bg-brand-muted/5 pl-14 pr-8 text-sm font-bold placeholder:text-brand-muted/40 focus-visible:ring-brand-primary/20"
                  value={searchQuery}
              ...
              <div className="flex items-center gap-6 w-full md:w-auto">
                ...
                >
                  <SelectTrigger className="h-14 w-full md:w-56 rounded-full border-none bg-brand-muted/5 font-bold font-heading text-xs text-brand-primary focus:ring-brand-primary/20">
                    <SelectValue placeholder="Sort Fleet" />
                  </SelectTrigger>
                  <SelectContent className="rounded-2xl border-none shadow-2xl p-2">
                    <SelectItem value="premium" className="text-xs font-bold font-heading py-3 rounded-xl transition-all">Premium Fleet First</SelectItem>
                ...
                <span className="shrink-0 text-xs font-bold font-heading text-brand-muted uppercase tracking-[0.2em] opacity-60">
            ...
            {filteredVehicles.length > 0 ? (
              ...
              <div className="flex min-h-[500px] flex-col items-center justify-center rounded-[3rem] border-2 border-dashed border-brand-muted/10 bg-white/30 p-16 text-center backdrop-blur-xl">
