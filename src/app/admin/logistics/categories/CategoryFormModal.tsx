...
import { upsertLogisticsCategory, deleteLogisticsCategory } from "../../actions";
import { LogisticsCategorySchema } from "../../schemas";

type CategoryFormValues = z.infer<typeof LogisticsCategorySchema>;
...
export default function CategoryFormModal({ category, trigger }: CategoryFormModalProps) {
  ...
  const form = useForm<CategoryFormValues>({
    resolver: zodResolver(LogisticsCategorySchema),
  ...
  async function onSubmit(data: CategoryFormValues) {
    setLoading(true);
    try {
      await upsertLogisticsCategory(data);
  ...
  async function handleDelete() {
    ...
    setLoading(true);
    try {
      await deleteLogisticsCategory(category!.id, password);
      toast.success("Category and associated vehicles deleted");
      setOpen(false);
    } catch (error: any) {
        ...
        <DialogHeader>
          ...
          {isDeleting && (
            <DialogDescription className="text-red-600 font-medium mt-2">
              ⚠️ Warning: Deleting this category will permanantly remove <b>{vehicleCount} vehicles</b>.
            </DialogDescription>
          )}
        </DialogHeader>
          ...
          <div className="space-y-6 pt-4">
            ...
            <DialogFooter className="gap-2">
              ...
              <Button 
                ...
                disabled={loading || (vehicleCount > 0 && !password)}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g. VIP Escort" {...field} className="rounded-[10px]" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
